package com.example.JobAppV2.repo;
...
public class JobRepo {
    ...
    public JobPost updateJob(int id, JobPost job) {

        for (JobPost obj : jobs) {
            if (obj.getPostId() == id) {
                obj.setPostId(job.getPostId());
                obj.setPostProfile(job.getPostProfile());
                obj.setPostDesc(job.getPostDesc());
                obj.setReqExperience(job.getReqExperience());
                obj.setPostTechStack(job.getPostTechStack());
                return obj; // Found the object
            }
        }
        return null;
